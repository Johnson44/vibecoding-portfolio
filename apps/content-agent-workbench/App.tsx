...
import type { CompetitorSample, EducationAgentInput, EducationAgentOutput, EducationContentPlan, EsportsAgentInput, NewsAgentInput, NewsAgentOutput, ViralAgentInput, ViralAgentOutput, ViralProcessStep } from "@portfolio/contracts";
import {
...
type Route = "education-agent" | "esports-agent" | "viral-agent" | "news-agent";
...
function currentRoute(): Route {
  if (window.location.pathname.includes("news")) return "news-agent";
    ...
    <div className="app-shell">
      <aside className="sidebar">
        ...
        <nav aria-label="项目导航">
          ...
          <button className={route === "news-agent" ? "active" : ""} onClick={() => navigate("news-agent")}>
            <span>04</span> 新闻核查
          </button>
        </nav>
        <div className="status-card">
          <span className="status-dot" /> {cloudConfig.enabled ? "CloudBase体验模式" : "研究数据模式"}
          <small>{cloudConfig.enabled ? "新增 Agent 优先使用云端严格JSON，失败自动回退" : "真实公开材料已接入；模型调用使用本地确定性输出"}</small>
        </div>
      </aside>
      <main>
        <header className="topbar">
          <div>
            ...
            <h2>{route === "education-agent" ? "升学图文与留资 Agent" : route === "esports-agent" ? "电竞视频与下单 Agent" : route === "viral-agent" ? "爆款内容拆解 Agent" : "新闻事实核查与摘要 Agent"}</h2>
          </div>
          <span className={`mode-pill ${cloudConfig.enabled ? "cloud" : "research"}`}>{cloudConfig.enabled ? "CLOUD MODE" : "RESEARCH MODE"}</span>
        </header>
        {route === "education-agent" ? <EducationPage /> : route === "esports-agent" ? <EsportsPage /> : route === "viral-agent" ? <ViralPage /> : <NewsPage />}
      </main>
    </div>
  );
}
...
function isNewsAgentOutput(value: unknown): value is NewsAgentOutput {
  if (!value || typeof value !== "object") return false;
  const candidate = value as Partial<NewsAgentOutput>;
  return typeof candidate.title === "string"
    && typeof candidate.summary === "string"
    && typeof candidate.confidence === "number"
    && Array.isArray(candidate.claims)
    && candidate.claims.every((claim) => Boolean(claim?.text) && ["verified", "needs-human-check", "conflict", "unsupported"].includes(claim.status))
    && Array.isArray(candidate.sources)
    && Array.isArray(candidate.riskFlags)
    && Array.isArray(candidate.manualChecks);
}
...
function NewsPage() {
  const [input, setInput] = useState<NewsAgentInput>({
    title: "台风“巴威”预警信息如何避免被误读",
    body: "中国气象局提示，台风“巴威”预警信息需要结合发布时间和影响地区判断。网传台风“巴威”已经确定登陆上海，暂无官方来源。",
    sourceIds: newsSources.map((source) => source.id)
  });
  const [submitted, setSubmitted] = useState(input);
  const localOutput = useMemo(() => runNewsAgent({ ...submitted, sources: newsSources }), [submitted]);
  const cloudConfig = useMemo(() => getCloudbaseConfig(), []);
  const [cloudOutput, setCloudOutput] = useState<NewsAgentOutput | null>(null);
  const [apiState, setApiState] = useState<"local" | "loading" | "cloud" | "fallback">("local");
  const [actionNote, setActionNote] = useState("");
  const output = cloudOutput ?? localOutput;
  const verifiedCount = output.claims.filter((claim) => claim.status === "verified").length;
  const unresolvedCount = output.claims.length - verifiedCount;
  const statusLabels = {
    verified: "已核验",
    "needs-human-check": "待人工复核",
    conflict: "来源冲突",
    unsupported: "缺少来源"
  } as const;
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    setApiState("loading");
    try {
      const response = await callAigcFunction<NewsAgentOutput>("newsAgent", { ...input, sources: newsSources });
      if (!isNewsAgentOutput(response.data)) throw new Error("云端核查结果未通过主张和来源字段校验");
      setCloudOutput(response.data);
      setApiState("cloud");
    } catch (error) {
      setApiState("fallback");
  ...
  return (
    <section className="workspace-grid news-workspace">
      <form className="panel form-panel news-form" onSubmit={(event) => void submit(event)}>
        <div className="panel-heading"><span>新闻素材输入</span><small>SOURCE INPUT</small></div>
        <FieldLabel>标题<input value={input.title} onChange={(event) => setInput({ ...input, title: event.target.value })} /></FieldLabel>
        <FieldLabel>正文 / 热点材料<textarea rows={12} value={input.body} onChange={(event) => setInput({ ...input, body: event.target.value })} placeholder="输入需要核验的新闻事实或热点文本" /></FieldLabel>
        <div className="material-box"><b>{newsSources.length} 条公开来源样本</b><span>政策、气象、健康三类主题</span><small>第一版不自动抓取实时内容；来源仅作为演示数据，发布前需人工复核。</small></div>
        <button className="primary-button news-button" type="submit" disabled={apiState === "loading"}>{apiState === "loading" ? "CloudBase核查中…" : "开始核查与摘要"} <span>→</span></button>
        <p className="form-note">{cloudConfig.enabled ? (apiState === "cloud" ? "已使用 CloudBase 云函数；事实状态和摘要仍需编辑复核。" : apiState === "fallback" ? actionNote : "已配置 CloudBase，提交后将优先调用云端。") : "当前为本地研究模式；无来源的主张不会被写成确定性结论。"}</p>
      </form>

      <div className="results-stack">
        <StatStrip items={[
          { value: output.claims.length, label: "提取事实主张" },
          { value: verifiedCount, label: "已核验主张" },
          { value: unresolvedCount, label: "待人工处理" },
          { value: `${output.confidence}%`, label: "当前证据覆盖" }
        ]} />
        <article className="panel news-summary-panel">
          <div className="panel-heading"><span>编辑摘要</span><small>EDITORIAL BRIEF</small></div>
          <p className="news-summary">{output.summary}</p>
          <div className="news-meta-row"><span>模式：{output.mode === "research" ? "本地研究数据" : "云端模型"}</span><span>{output.title}</span></div>
        </article>
        <article className="panel claim-panel">
          <div className="panel-heading"><span>事实主张与证据状态</span><small>CLAIM AUDIT</small></div>
          <div className="claim-list">
            {output.claims.map((claim) => <article className="claim-row" key={claim.id}>
              <div className="claim-head"><span className={`claim-status ${claim.status}`}>{statusLabels[claim.status]}</span><b>{claim.text}</b></div>
              <p>{claim.rationale}</p>
              <div className="claim-evidence"><span>证据</span>{claim.evidence.map((item) => <em key={item}>{item}</em>)}</div>
            </article>)}
          </div>
        </article>
        <div className="split-results">
          <article className="panel source-panel"><div className="panel-heading"><span>公开来源</span><small>SOURCES</small></div>{output.sources.map((source) => <a className="source-row" href={source.url} target="_blank" rel="noreferrer" key={source.id}><span className="verified-dot" /><div><b>{source.title}</b><small>{source.publisher} · {source.publishedAt} · {source.scope}</small></div></a>)}</article>
          <article className="panel risk-panel news-risk"><div className="panel-heading"><span>发布前人工闸门</span><small>HUMAN REVIEW</small></div><div className="risk-grid">{[...output.riskFlags, ...output.manualChecks].map((item) => <span key={item}>{item}</span>)}</div></article>
        </div>
      </div>
    </section>
  );
